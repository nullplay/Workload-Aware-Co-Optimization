...
Stmt LowererImpl::lowerForall(Forall forall)
{
  ...
  if (lattice.iterators().size() == 1 && lattice.iterators()[0].isUnique()) {
    ...
    // Emit dimension coordinate iteration loop
      ...
      //[WJY]
      /*std::stringstream s;
      s << forall.getIndexVar();
      if(s.str() == "j1bound") {
        for (auto& a : appenders) { cout << a << " "; }
        cout <<endl;
        for (auto& a : inserters) { cout << a << " "; }
        cout <<endl;
        for (auto& a : point.locators()) { cout << a << " "; }
        cout <<endl;
        for (auto& a : reducedAccesses) { cout << a << " "; }
        cout << endl;
  ...
  for (int i = (int) underivedAncestors.size() - 2; i >= 0; i--) {
    ...
    if (parallelUnitIndexVars.count(ParallelUnit::GPUBlock)) {
      ...
      if (i == (int) underivedAncestors.size() - 2) {
        std::string idxh("copyhost");
        header.push_back(ir::VarDecl::make(ir::Var::make(idxh, taco::Int32),  ir::Call::make("", {ir::Literal::make(posIteratorLevel.getMode().getLevel()-1)}, taco::Int32)));

        std::string idxd("binary_index_device");
        std::string idxsize("sizeof(int)");
        std::vector<Expr> cudaArgs = {
                ir::Var::make(idxd, taco::Int32),
                ir::Var::make("&"+idxh, taco::Int32),
                ir::Var::make(idxsize, taco::Int32)
        };
        std::string err("err");
        header.push_back(ir::VarDecl::make(ir::Var::make(err, taco::Int32),  ir::Call::make("cudaMemcpyToSymbol", cudaArgs, taco::Int32)));

        std::string Ad("Ad");
        std::vector<Expr> args = {
                //posIteratorLevel.getMode().getModePack().getArray(0), // array
                ir::Var::make(Ad, taco::Int32), // array
